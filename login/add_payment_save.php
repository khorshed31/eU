...
// var_dump($conn);
// exit;       
$uploaded_by_query = mysqli_query($conn,"select * from student where student_id = '$session_id'");
...
$teacher = $teacher_query_row['teacher_class_id'];

//$subject= clean($_POST['upname']);

if ($subject == '') {
    $errmsg_arr[] = ' Course is missing';
    $errflag = true;
}
...
if ($errflag) {
   ...
   <script>
   window.location = 'add_payment.php<?php echo '?id='.$id;  ?>';
   </script>
   <?php exit();
}
...
if ((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
    ...
    if (($ext != "exe") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload")) {
		...
		$name_notification  = 'Add Payment Photo'." ".'<b>'.$name.'</b>';
        ...
        if (!file_exists($newname)) {
            ...
            if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
               ...
               // echo "INSERT INTO files (fdesc,floc,fdatein,teacher_id,class_id,fname,uploaded_by) VALUES ('$filedesc','$newname',NOW(),'$session_id','$id[$i]','$name','$uploaded_by')";
                // exit;
				mysqli_query($conn,"INSERT INTO payment (pay_image,date,student_id,subject_id,method,teacher_class_id,trans_id,uploaded_by) VALUES ('$newname',NOW(),'$session_id','$subject','$met','$teacher','$trans','$uploaded_by')");
